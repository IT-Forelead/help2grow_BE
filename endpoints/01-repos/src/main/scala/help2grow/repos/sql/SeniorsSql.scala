...
private[repos] object SeniorsSql extends Sql[PersonId] {
  ...
  val codec =
    (id *: zonedDateTime *: nes *: nes *: role *: email *: zonedDateTime.opt *: zonedDateTime.opt *: nes *: nes *: SkillsSql.id *: nes)
      .to[SeniorUser]
  ...
  def get(filters: SeniorFilters): AppliedFragment = {
    val baseQuery: Fragment[Void] =
      sql"""
         SELECT
           u.id, u.created_at, u.firstname, u.lastname, u.role, u.email, u.updated_at, u.accepted_at, s.github, s.linked_in, s.main_skill, s.suggestion
         FROM seniors s
